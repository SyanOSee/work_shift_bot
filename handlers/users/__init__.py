...
users_router = Router()
[users_router.sub_routers.append(router) for router in [
...
async def handle_decline_message(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(text='<b>Состояние команды сброшено!</b>\n\nВоспользуйтесь командой <i>/help</i> или <i>/menu</i> для дальнейших действий', reply_markup=ReplyKeyboardRemove())
